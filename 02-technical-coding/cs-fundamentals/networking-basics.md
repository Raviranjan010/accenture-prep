...
2. **Encapsulation**: As data moves down network layers, each layer adds its own header (Application $ightarrow$ Transport $ightarrow$ Network $ightarrow$ Data Link $ightarrow$ Physical).
...
2. **DNS Resolution**:
   ...
   - Queries ISP Recursive DNS Server $ightarrow$ Root DNS $ightarrow$ TLD Server $ightarrow$ Authoritative Server $ightarrow$ Returns IP Address.
...
- **Mnemonic (Top to Bottom)**: **A**ll **P**eople **S**eem **T**o **N**eed **D**ata **P**rocessing (*Application $ightarrow$ Presentation $ightarrow$ Session $ightarrow$ Transport $ightarrow$ Network $ightarrow$ Data Link $ightarrow$ Physical*).

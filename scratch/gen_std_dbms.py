...
- **Rule**: Must be in **2NF** AND have **no transitive dependency** (non-key columns must NOT depend on other non-key columns: $A \rightarrow B$ and $B \rightarrow C$).
- **Non-3NF Table (Bad)**:
  | Student_ID (PK) | Zip_Code | City |
  | :--- | :--- | :--- |
  | 101 | 144411 | Phagwara |
  *Problem*: `City` depends on `Zip_Code`, which depends on `Student_ID` ($Student\_ID \rightarrow Zip\_Code \rightarrow City$).
- **Realistic Enterprise Example (3NF Split)**:
  1. `Students(Student_ID, Zip_Code)`
  2. `ZipCodes(Zip_Code, City)`

#### BCNF (Boyce-Codd Normal Form)
- **Rule**: A stricter version of 3NF. For every functional dependency $X \rightarrow Y$, $X$ MUST be a **super key**.
...
- **Rule**: Remember the sequence: $1NF \rightarrow 2NF \rightarrow 3NF \rightarrow BCNF$.
  - 1NF = Atomic values.
...
9. **(Hard - Scenario)** Consider functional dependencies $A \rightarrow B$ and $B \rightarrow C$ in a table with Primary Key $A$. Which normal form does this violate, and how do you fix it?
...
8. **Answer**:
   ...
   ```
9. **Answer**: Violates **3NF** due to transitive dependency ($A \rightarrow B \rightarrow C$). Fix by splitting into two tables: `Table1(A, B)` and `Table2(B, C)`.
...
print("dbms-normalization-joins.md updated.")
